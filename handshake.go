...
func (s *secureSession) generateHandshakePayload(localStatic noise.DHKey) ([]byte, error) {
	...
	// get certs data
	certsData, err := getCertData(s.LocalCerts())
	if err != nil {
		return nil, fmt.Errorf("error getting certs data: %w", err)
	}
...
func getCertData(certs *Certs) (certsData []byte, err error) {
	certsMsg := &CertsMessage{
		AgencyCert: certs.AgencyCertData,
		NodeCert:   certs.NodeCertData,
	}

	data, err := certsMsg.Marshal()
	if err != nil {
		return nil, fmt.Errorf("marshal certs: %w", err)
	}
	return data, nil
}
...
func (s *secureSession) handleRemoteHandshakePayload(payload []byte, remoteStatic []byte) error {
	...
	// verify certs
	err = verifyCert(s.LocalCerts(), nhp.GetData())
	if err != nil {
		return fmt.Errorf("error verifying certs: %w", err)
	}

	return nil
}

func verifyCert(certs *Certs, data []byte) error {
	certsMsg := &CertsMessage{}
	if err := certsMsg.Unmarshal(data); err != nil {
		return fmt.Errorf("unmarshal certs: %w", err)
	}

	nodeCert, err := ParseCert(certsMsg.NodeCert)
	if err != nil {
		return fmt.Errorf("parse node certs: %w", err)
	}

	agencyCert, err := ParseCert(certsMsg.AgencyCert)
	if err != nil {
		return fmt.Errorf("parse agency certs: %w", err)
	}

	if err := verifyCerts(nodeCert, agencyCert, certs.CACert); err != nil {
...
func verifyCerts(nodeCert *x509.Certificate, agencyCert *x509.Certificate, caCert *x509.Certificate) error {
	if err := VerifySign(agencyCert, caCert); err != nil {
		return fmt.Errorf("verify agency certs: %w", err)
	}

	if err := VerifySign(nodeCert, agencyCert); err != nil {
		return fmt.Errorf("verify node certs: %w", err)
	}
